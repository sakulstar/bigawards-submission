...
let placeholderHeader = "biggest boomer moment"
let placeholderDisc = "Moments from past streams that made you go :BOOMER:"
...
function sendSubmit() {
    ...
    fetch('https://id.twitch.tv/oauth2/validate', twitchValidate)
        ...
        const categoryText: any = document.getElementById("category")!.textContent
        ...
        const options = {
            ...
            body: `{"content": "**Category:** ${categoryText}\\n**Username:** ${response["login"]}\\n**Nominee:** ${nomineeText}\\n**Their case:** ${caseText}"}`
        };
...
export default function submit() {
    return (
        <div className="relative">
            <div className="rounded-t-3xl absolute inset-x-0 top-0 h-6 bg-red-500"></div>
            <div className="max-sm:hidden max-w-[434px] min-w-[434px] max-h-[528px] min-h-[528px] max-sm:max-w-[350px] max-sm:min-w-[350px] bg-dark-blue px-10 py-6 rounded-3xl shadow-2xl overflow-hidden">
                <p id="category" className="break-words capitalize text-4xl pt-8 max-sm:pt-5 mb-2 max-sm:mb-1 text-center">{placeholderHeader}</p>
                <p className={`${robotoRegular.className} break-words capitalize text-md text-center`}>{placeholderDisc}</p>
